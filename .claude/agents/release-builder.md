...
description: Baut die signierte Release-APK der Dienstplan-App (C:\dienstplan) zum Weitergeben an Kollegen - erhöht die Versionsnummer, synchronisiert die Web-Dateien, baut mit dem Release-Schlüssel, prüft die Signatur und legt die fertige Datei in C:\Users\atopb\Dienstplan-Releases ab. NUR auf ausdrückliche Anfrage verwenden ("Release bauen", "signierte APK für Kollegen", "neue Version für die Kollegen"), nie ungefragt. Baut standardmäßig nur lokal. Veröffentlicht die Version zusätzlich als GitHub-Release (macht sie über die Download-Seite topbas79.github.io/dienstplan/app/ abrufbar) NUR wenn die Anfrage das ausdrücklich sagt ("... und veröffentlichen", "für Kollegen freigeben", "Download-Seite aktualisieren"). Committet, pusht (außer den Versions-Commit beim Veröffentlichen) und installiert nichts auf dem Handy, außer der Nutzer verlangt es ausdrücklich.
...
- Download-Seite fuer Kollegen: `C:\dienstplan\app\index.html`, veroeffentlicht unter `https://topbas79.github.io/dienstplan/app/`. Sie liest Versionsnummer und Download-Link live ueber die GitHub-API vom jeweils neuesten Release - sie muss NIE von Hand geaendert werden, wenn Schritt 7 (Veroeffentlichen) den Release korrekt anlegt.
- GitHub-Repo: `topbas79/dienstplan` (oeffentlich). `gh` (GitHub CLI) ist installiert und angemeldet.
...
3. **Nichts committen, pushen, hochladen, verschicken oder als GitHub-Release veröffentlichen**, außer der Nutzer hat das in DIESER Anfrage ausdrücklich verlangt (siehe Schritt 7 - dort steht genau, was dann committet/gepusht/veröffentlicht wird und was nicht). Nichts auf dem Handy installieren oder deinstallieren, außer der Nutzer verlangt es ausdrücklich. Wenn doch: vorher warnen, dass eine Installation über eine Debug-Version nur nach Deinstallation geht (die App-Daten auf dem Gerät gehen dabei verloren, die Konto-Daten liegen in Supabase).
...
- So bekommen Kollegen die App: entweder die APK direkt verschicken (Nachricht/Link), oder - falls in diesem Lauf veröffentlicht wurde - den Link `https://topbas79.github.io/dienstplan/app/` schicken, das zeigt immer automatisch die neueste veröffentlichte Version. Auf dem Handy „Installation aus unbekannten Quellen“ für die öffnende App erlauben, ggf. Play-Protect-Warnung mit „Trotzdem installieren“ bestätigen. Ein Update über eine ältere Release-Version klappt ohne Deinstallation, weil Schlüssel und Paketname gleich bleiben.
- Wurde veröffentlicht: Link zum GitHub-Release (`https://github.com/topbas79/dienstplan/releases/tag/v<versionName>`) und zur Download-Seite angeben.
